...
|Current WBS|`DOC-01-A02 内部受权 Document 元数据读取`（Windows 11/隔离 PostgreSQL 合成验证）|
...
|Test Summary|DOC-01-A02：Windows 11/Python 3.13 后端 508 项无失败（2 项符号链接场景跳过）；隔离 PostgreSQL 18 当前 Session/项目成员/GLOBAL 管理员、keyset、归档、受限/跨项目/停用成员与 License 验证、开发 wheel PASS。未操作生产库|
|Next WBS|DOC-01-A03 独立签名 Document 列表 cursor 与可选 GET 契约前置；后续 Windows 组合、版本读取和受权流式下载。P03-P02 物理清理待停写栅栏，Phase 3 才做正式 Parser Worker；生产信任源/Server 2025 仍待，Debian 13 暂不验证|
