...
|Test Summary|DOC-01-A03-P04：Windows 11/Python 3.13 后端 518 项无失败（2 项既有符号链接环境跳过）；Windows 平台合同 17 项 PASS，独立 Document 游标密钥缺失拒绝启动；开发 wheel PASS。前序隔离 PostgreSQL 18.6 HTTP 同链路 PASS（合成 License）。未操作生产库|
|Next WBS|DOC-01-A04 DocumentVersion 元数据读取与分页前置检查；随后受权流式下载。P03-P02 物理清理待停写栅栏，Phase 3 才做正式 Parser Worker；生产信任源/Server 2025 仍待，Debian 13 暂不验证|
