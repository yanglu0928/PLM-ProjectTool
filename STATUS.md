...
|Current WBS|`EVD-01-A03-P01 Evidence 候选创建授权边界`（Windows 11 权限/回归 PASS；A03 整体仍待来源证明）|
...
|Test Summary|EVD-01-A03-P01：Windows 11/Python 3.13 后端 568 项无失败（2 项既有符号链接环境跳过）；角色/会话/Scope 拒绝与开发 wheel PASS。尚无真实文档定位、候选写库或生产权限组合；EVD-A02 隔离数据库验证历史结果保持|
|Next WBS|EVD-01-A03-P02 DocumentService 固定版本与九型 Locator 来源证明 Port；之后 P03 同事务候选创建/幂等/Audit。CR-DOC-008/A03-P03 待目标账户/停写材料，Debian 13 暂不验证|
