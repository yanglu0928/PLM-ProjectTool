...
|Current WBS|`DOC-04-A03 ParseRecord 列表可选 HTTP`（合成契约与 Windows 11 回归 PASS；默认未挂载）|
...
|Test Summary|DOC-04-A03：Windows 11/Python 3.13 后端 578 项无失败（2 项既有符号链接环境跳过）；合成 HTTP 双 Scope、会话/资源/版本绑定游标、脱敏投影、异常和开发 wheel PASS；A02 隔离 PostgreSQL 18.6 内部读取通过。尚无真实 PostgreSQL HTTP 联调、正式 Parser 或结果文件|
|Next WBS|DOC-04-A04 Windows 当前账户独立 Parse 游标密钥来源与恢复验证；随后 A05 做平台组合及 PostgreSQL HTTP 端到端。EVD-01-A03-P02-A02 待 Phase 3 正式 Parser 结果恢复，Debian 13 暂不验证|
