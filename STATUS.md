...
|Current WBS|`RVW-02-A08 可信事务决定/撤回`（完整 owned 结构/真实 Audit/合成 Owner 消费及回滚 PASS；受权幂等入口和实际 Owner 待实施）|
...
|Test Summary|RVW-02-A08 Windows 11/Python 3.13 后端 751 项无失败（2 项环境跳过）；隔离 PostgreSQL 决定/撤回/历史/原因/真实 Audit、完整集合终态、消费写后重核/审计故障全回滚、最终决定对撤回竞争单终态；0035/受权送审回归与开发 wheel PASS。Owner 仅合成事务标记，无本轮受权命令/receipt/HTTP/实际批准/Gate 证明|
|Next WBS|RVW-02-A09 当前 Session/CSRF/Project/assigned reviewer 或 PM 权限与持久幂等决定/撤回内部入口，完成锁序/历史重放/失败关闭与全事务恢复。缺真实 Owner 不挂载 HTTP，完整 Scope 保留；发行 Gate/Server 2025 待验证，Debian 13 暂不验证|
