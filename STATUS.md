...
|Current WBS|`DOC-03-A03-P03 文件完整性校验与受控 AVAILABLE 发布`（内部命令已合成验证；崩溃恢复/公开 HTTP 未实现）|
|Current Status|PHASE_1_COMPLETE / DOC_03_A03_P03_INTERNAL_PASS / DOC_03_A03_RECOVERY_PENDING / PLT_02_A07_P05_A09_CEREMONY_PENDING / PHASE_2_IN_PROGRESS|
...
|Test Summary|DOC-03-A03-P03：Windows 11/Python 3.13 后端 456 项无失败（2 项符号链接场景跳过）；PostgreSQL 18 临时库与合成文件验证 Hash/Size、Scope/权限、重放及审计失败回滚、开发 wheel PASS|
|Next WBS|DOC-03-A03-P04 发布中断后的受控恢复/隔离；随后 DOC-02 内部版本提交协调；发行信任源仍待工作台操作员仪式，Server 2025/HTTPS 仍待；Debian 13 暂不验证|
