...
|Current WBS|`AUD-03-A06-A04-P03-A07-P04-P03-P06-P01 执行状态事实`（原pair/当前与历史代次/实际Attempt-Lease只读内部PASS；执行器接线/主循环待）|
...
|Test Summary|P06-P01 Windows11/Python3.13后端1047项无失败（2既有环境跳过）；5新unit/实际PG-Vault两Scope原pair/当前与旧代/实际状态-租约-DB clock六表只读，真实新claim/成功/失败/取消/到期；撤User+License仍业务拒绝，错绑定/后identity拒绝；旧发布/wheel成功。新unit漏必填参数修正后重跑|
|Next WBS|AUD-03-A06-A04-P03-A07-P04-P03-P06-P02：单命令执行器成功/终止/取消/重试及来源核验接线；再主循环/提交Jobs HTTP。公开POST/If-Match仍待。网络/质量/Gate/正式材料/完整Scope保留，Server2025未验、Debian13暂缓|
