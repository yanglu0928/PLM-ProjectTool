...
|Current WBS|`EVD-01-A02 Evidence 固定版本持久模型`（隔离 PostgreSQL 18.6 与 Windows 11 回归 PASS；未接创建/Viewer）|
...
|DB Schema Version|`DB-SCHEMA-CANDIDATE-V1`；Gate 2 原冻结内容 `64cdf09`；增量至 `20260926_0027` Evidence 固定版本记录，前序 `0026` Outbox 投递租约（CR-DOC-007）与 0001～0025 历史保留|
|API Contract Version|`API-CONTRACT-CANDIDATE-V1`；API-01～API-05 PASS，Gate 2 已冻结（内容提交 `64cdf09`）|
|Test Summary|EVD-01-A02：Windows 11/Python 3.13 后端 564 项无失败（2 项既有符号链接环境跳过）；隔离 PostgreSQL 18.6 空/已有版本升级、空表降级、ORM parity、双 Scope、来源/历史约束和开发 wheel PASS。未验证真实文档定位或生产权限；CR-DOC-008 生产删除仍关闭|
|Next WBS|EVD-01-A03 内部受权候选创建/固定版本来源解析 Port；CR-DOC-008/A03-P03 待正式目标账户/停写材料恢复。POC-03 质量与 Release 容量仍待，Debian 13 暂不验证|
