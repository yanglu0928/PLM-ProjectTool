...
|Current WBS|`AUD-03-A06-A04-P03-A04-P02 真实私有文件渲染`（短事务页取/事务外暂存与Hash/当前权限Lease末验PASS；实际原子发布待）|
...
|Test Summary|AUD-03-A06-A04-P03-A04-P02 Windows11/Python3.13后端930项无失败（2环境跳过）；真实受理/claim/capture/权限Lease→128页/事务外文件写fsync/hash→尾验，双Scope empty/260多页/中文存储根/新事件排除/撤权取消到期失败私有及新代独立file、计划/renderer/结果回归与wheel PASS。License/ENOSPC合成，无final/元数据/成功发布/HTTP/心跳/性能/发行证明|
|Next WBS|AUD-03-A06-A04-P03-A04-P03：真实Document metadata/受控提升、own Result/发布Audit/Jobs caller-UOW完成原子发布，权限/Lease/取消再核，再受控恢复与访问授权。POST关闭；真实Owner/质量/Gate/正式材料仍待，完整Scope保留，Server2025未验、Debian13暂缓|
