...
|Current WBS|`AUD-03-A06-A03 受权Worker capture`（实际受理/原pair/claim/当前权限/Lease/capture全UOW、故障并发/40P01 PASS；License合成，文件/HTTP未验收）|
...
|DB Schema Version|`DB-SCHEMA-CANDIDATE-V1`；原冻结64cdf09保留；增量至`20260926_0039` Job首次取消信息（CR-JOB-002），0001～0038历史不改写；无生产迁移|
|API Contract Version|`API-CONTRACT-CANDIDATE-V1`；API-01～API-05 PASS，Gate 2 已冻结（内容提交 `64cdf09`）|
|Test Summary|AUD-03-A06-A03 Windows11/Python3.13后端873项无失败（2环境跳过）；真实受理/当前权限/原pair/Lease/capture、重复并发/撤权/实际取消/故障/到期接管/新generation/真实40P01及相关回归、开发wheel PASS。License合成；渲染/文件交付/性能未验，发行未完成|
|Next WBS|AUD-03-A06-A04：固定seal安全投影/JSONL渲染、Artifact受控存储/空间/归属/Hash与生命周期；随后权限/Lease/取消下原子发布及用户取消receipt/Audit/HTTP和访问再授权。POST关闭；真实Owner/质量/Gate/正式材料仍待，完整Scope保留，Server2025未验、Debian13暂缓|
