...
|Current WBS|`AUD-03-A03 导出请求与授权合同`（固定用途/Scope/筛选指纹及Worker权限Port内部合同PASS；实际权限/持久化/导出HTTP未完成）|
...
|Test Summary|AUD-03-A03 Windows11/Python3.13后端805项无失败（2项环境跳过）；纯内部Spec范围/用途/时间/筛选/UTC指纹、冻结最小字段/Worker阶段坐标合同与开发wheel PASS。本轮无新增真实权限/数据库/HTTP/导出文件验收；正式发行未完成|
|Next WBS|AUD-03-A04 / CR-AUD-001：不可变导出/capture Schema实施前变更记录、根实体/来源固定与seal/迁移验证，后续真实权限与Job编排；完整导出按A01设计。Review真实Owner缺失保持公开关闭，完整Scope保留。发行Gate/Server2025待验证，Debian13暂不验证|
