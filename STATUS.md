...
|Current WBS|`AUD-03-A06-A04-P03-A07-P04-P03-P04-P01 取消申请权限`（真实创建者或PM/Scope/Session矩阵PASS；取消Owner/接线/主循环待）|
...
|Test Summary|P04-P04-P01 Windows11/Python3.13后端1012项无失败（2既有环境跳过）；4新unit/真实PG双Scope Session-CSRF-User-role-License、PROJECT creator所有当前有效角色或PM允许/非creator普通member跨项目Admin无member暂停拒绝、Archived只停止权限，九表每次无写；原发布/wheel成功。权限Port非取消完成|
|Next WBS|AUD-03-A06-A04-P03-A07-P04-P03-P04-P02：取消申请Owner实际Root/当前权限/同UOW首申请历史-USER Audit来源与原结果幂等；再系统确认/到期恢复、瞬时retry/执行器/主循环/提交Jobs HTTP。POST仍关闭。网络/质量/Gate/正式材料/完整Scope保留，Server2025未验、Debian13暂缓|
