...
|Current WBS|`AUD-03-A06-A04-P02 文件归属前置核查`（4项只读入口探测PASS；现有DEPLOYMENT文件发布不兼容，须专项CR后实施）|
...
|Next WBS|AUD-03-A06-A04-P03：先专项CR记录DEPLOYMENT FileObject内部归属与Audit自有结果契约，保留Document/Upload/Parse原范围，再受控存储/元数据/恢复与原子发布验收。随后用户取消receipt/Audit/HTTP及下载再授权。POST关闭；真实Owner/质量/Gate/正式材料仍待，完整Scope保留，Server2025未验、Debian13暂缓|
