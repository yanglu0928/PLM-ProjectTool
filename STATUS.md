...
|Current WBS|`AUD-03-A06-A04-P03-A07-P04-P03-P04-P04 到期取消恢复`（严格当前代DB到期/SYSTEM Audit同UOW内部PASS；确认丢失/接线/主循环待）|
...
|Test Summary|P04-P04-P04 Windows11/Python3.13后端1025项无失败（2既有环境跳过）；4新unit/实际PG-Vault两Scope严格当前Worker/fence真实到期恢复SYSTEM Audit同UOW，未到期/旧代/终态/裸技术来源拒绝，Audit/恢复写后及identity故障回滚，撤User+License业务拒绝仅安全恢复，首历史/字节/结果保留；旧发布/wheel成功|
|Next WBS|AUD-03-A06-A04-P03-A07-P04-P03-P04-P05：取消提交确认丢失只读源核验；再瞬时retry/执行器/主循环/提交Jobs HTTP。公开POST/If-Match仍待。网络/质量/Gate/正式材料/完整Scope保留，Server2025未验、Debian13暂缓|
