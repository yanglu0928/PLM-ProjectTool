...
|Current WBS|`AUD-03-A06-A02-P02-A02 内部协作取消`（Jobs实际取消/Worker确认/到期恢复/完成竞争PASS；Owner授权/receipt/Audit/文件/HTTP未验收）|
...
|Test Summary|AUD-03-A06-A02-P02-A02 Windows11/Python3.13后端866项无失败（2环境跳过）；真实取消/当前Worker确认/到期并发恢复/首次信息/故障回滚/取消与finish实际等待竞争、相关回归与开发wheel PASS。Owner权限/真实文件未接入；完整Worker/交付/性能未验，发行未完成|
|Next WBS|AUD-03-A06-A03：实际Root/acceptance/原Job pair、当前权限、Lease/取消与capture短事务编排；随后文件/发布/访问再授权及用户取消receipt/Audit/HTTP。POST关闭；真实Owner/质量/Gate/正式材料仍待，完整Scope保留，Server2025未验、Debian13暂缓|
