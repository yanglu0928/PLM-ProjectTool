...
|Current WBS|`DOC-03-A04-A03-P04-P02-A02 独立上传 Token Windows 密钥来源`（Windows 11 合成恢复验证；生产组合未装配）|
...
|Test Summary|DOC-03-A04-A03-P04-P02-A02：Windows 11/Python 3.13 后端 489 项无失败（2 项符号链接场景跳过）；独立测试凭据失密/加密备份恢复后旧上传 Token 重新派生、缺钥拒绝及开发 wheel PASS。目标账户正式引用未供给，HTTP+数据库组合未验证|
|Next WBS|DOC-03-A04-A03-P04-P02-A03 将可选 UploadIntent 创建 HTTP 装入 Windows 显式平台组合并验证真实 Session/License/角色；随后 Content HTTP、Commit/Abort、Parser Job/Outbox；生产发行信任源/Server 2025 仍待，Debian 13 暂不验证|
