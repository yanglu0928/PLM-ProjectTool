...
|Test Summary|DOC-03-A04-A03-P04-P02-A01：Windows 11/Python 3.13 后端 487 项无失败（2 项符号链接场景跳过）；可选 HTTP 契约、默认 404、隔离 PostgreSQL 18 原 UploadIntent 创建/父版本拒绝回归及开发 wheel PASS。真实 Session+License+HTTP+数据库组合未验证|
|Next WBS|DOC-03-A04-A03-P04-P02-A02 建立独立上传 Token 密钥的 Windows 当前账户安全来源/备份恢复，然后将可选 UploadIntent 创建 HTTP 装入显式平台组合并验证真实 Session/License/角色；随后 Content HTTP、Commit/Abort、Parser Job/Outbox；生产发行信任源/Server 2025 仍待，Debian 13 暂不验证|
