...
|Current WBS|`DOC-03-A04-A03-P03-A02-P03-P01 单候选 TTL 清理`（Windows 11 合成文件/PostgreSQL 已验证；全量扫描/公开 HTTP 未实现）|
...
|Test Summary|DOC-03-A04-A03-P03-A02-P03-P01：Windows 11/Python 3.13 后端 475 项无失败（2 项符号链接场景跳过）；PostgreSQL 18 临时库/合成文件七天单候选清理、活动锁/年龄/已登记拒绝、权限/审计/收据及开发 wheel PASS。仍无目录扫描、正式 Session 授权或 HTTP|
|Next WBS|DOC-03-A04-A03-P03-A02-P03-P02 受控暂存目录扫描与物理删除后审计中断对账；随后正式 Session/License/CSRF/Project Role 适配与公开 HTTP、Commit/Abort、Parser Job/Outbox；DOC-03-A03-P04 正式停写证明和 DOC-02 非上传来源待其 Owner；发行信任源/Server 2025 仍待，Debian 13 暂不验证|
