...
|Current WBS|`AUD-03-A06-A04-P03-A07-P04-P03-P04-P02 取消申请事务`（原Root/首申请USER Audit/持久原响应PASS；系统确认/接线/主循环待）|
...
|Test Summary|P04-P04-P02 Windows11/Python3.13后端1017项无失败（2既有环境跳过）；5新unit/真实PG两Scope申请状态-首USER Audit-收据同UOW、并发同key原响应/异payload冲突、后续CANCELLED仍重放首次CANCEL_REQUESTED八表无写、新key保首历史、PENDING立即取消/成功Result不撤回/裸技术取消拒绝；Audit/Job写后及后验auth失败回滚，旧发布/wheel成功|
|Next WBS|AUD-03-A06-A04-P03-A07-P04-P03-P04-P03：原首申请Audit来源的SystemActor安全确认；再过期恢复/取消确认丢失、瞬时retry/执行器/主循环/提交Jobs HTTP。公开POST/If-Match仍待。网络/质量/Gate/正式材料/完整Scope保留，Server2025未验、Debian13暂缓|
