...
|Current WBS|`AUD-03-A06-A02-P01 当前Lease检查`（真实三事实锁/到期接管/旧Worker/只检查不续租PASS；取消状态合成，完整Worker/取消/HTTP未验收）|
...
|Test Summary|AUD-03-A06-A02-P01 Windows11/Python3.13后端857项无失败（2环境跳过）；真实三事实Lease锁/无写/到期接管/旧Worker拒绝、原租约回归及开发wheel PASS。Owner坐标/取消状态合成；完整Worker/取消/交付/性能未验，发行未完成|
|Next WBS|AUD-03-A06-A02-P02：Jobs owned取消/协作确认/到期恢复事务Port，再做Audit受权取消与Root/acceptance/Job/current-authority/capture编排、文件/发布/访问再授权。POST关闭；真实Owner/质量/Gate/正式材料仍待，完整Scope保留，Server2025未验、Debian13暂缓|
