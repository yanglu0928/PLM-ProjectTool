...
|Current WBS|`DOC-04-A02 固定版本解析记录受权读取`（内部服务、隔离 PostgreSQL 18.6 与 Windows 11 回归 PASS；未运行 Parser）|
...
|Test Summary|DOC-04-A02：Windows 11/Python 3.13 后端 576 项无失败（2 项既有符号链接环境跳过）；隔离 PostgreSQL 18.6 同时间戳 keyset、Scope 隔离、安全投影及开发 wheel PASS。未运行正式 Parser、结果文件或精确 Evidence 定位|
|Next WBS|DOC-04-A03 冻结 DOCUMENT_PARSE_LIST 的可选 HTTP 契约与受控游标；EVD-01-A03-P02-A02 待 Phase 3 正式 Parser 结果恢复。CR-DOC-008/A03-P03 待目标账户/停写材料，Debian 13 暂不验证|
