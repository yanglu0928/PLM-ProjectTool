...
|Current WBS|`PRJ-04-A13-P04 Windows 显式平台 Department 列表组合`（隔离 PostgreSQL 已验证；默认模式未挂载）|
|Current Status|PHASE_1_COMPLETE / PRJ_04_A13_P04_WINDOWS_PLATFORM_PASS / PLT_02_A07_P05_A09_CEREMONY_PENDING / PHASE_2_IN_PROGRESS|
...
|DB Schema Version|`DB-SCHEMA-CANDIDATE-V1`；Gate 2 原冻结内容 `64cdf09`；Auth 限流增量 `20260925_0012`，PRJ-01～03 正式实现 Migration `20260925_0013`，成员变更历史增量 `20260925_0014`（CR-PRJ-001），通用幂等收据增量 `20260925_0015`（CR-API-001），成员首次响应快照增量 `20260925_0016`（CR-PRJ-002），成员状态首次响应快照增量 `20260925_0017`（CR-PRJ-003）|
|API Contract Version|`API-CONTRACT-CANDIDATE-V1`；API-01～API-05 PASS，Gate 2 已冻结（内容提交 `64cdf09`）|
|Test Summary|PRJ-04-A13-P04：Windows 11/Python 3.13 后端 430/430 PASS；PostgreSQL 18 临时库两种显式模式部门分页/权限/License/缺密钥拒绝，8 个 Project/Member/Department 集成脚本回归及开发 wheel PASS|
|Next WBS|PRJ-04-A14-P01 Department 创建持久幂等前置核查；PLT-02-A07-P05-A09 发行信任源待工作台操作员仪式，Server 2025/HTTPS 仍待；Debian 13 暂不验证|
