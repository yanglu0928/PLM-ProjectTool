...
|Current WBS|`AUD-03-A06-A01 Worker当前权限`（真实当前User/PM/成员/部门/Admin及每stage事实锁PASS；Export坐标与License合成，完整Worker/HTTP未验收）|
...
|Test Summary|AUD-03-A06-A01 Windows11/Python3.13后端853项无失败（2环境跳过）；真实三stage当前权限/撤权/归档/范围/四事实锁/业务无写、原子提交回归及开发wheel PASS。Export坐标与License合成；完整Worker/交付/性能未验，发行未完成|
|Next WBS|AUD-03-A06-A02：Jobs事务内当前Lease只检查公共Port与取消边界，再做Root/acceptance/Job/current-authority/capture编排、文件/发布/访问再授权。POST关闭；真实Owner/质量/Gate/正式材料仍待，完整Scope保留，Server2025未验、Debian13暂缓|
