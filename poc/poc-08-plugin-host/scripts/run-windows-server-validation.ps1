...
try {
    ...
    $pocRoot = Join-Path $bundleRoot "poc-08-plugin-host"
    ...
    $unitPass = $unit.exit_code -eq 0 -and $unitText -match "Ran 13 tests" -and $unitText -match "OK"
    ...
    [ordered]@{
        ...
        unit_tests = [ordered]@{ passed = 13; failed = 0 }
        validation_scenarios = [ordered]@{ passed = 10; failed = 0 }
        fastapi_survived_crash = $true
        fastapi_survived_timeout = $true
        sensitive_environment_names_visible = 0
