...
- 本地 OCR 规范化检索 Top-5 为 116/120（96.67%），但当时仅是诊断路径；用户明确授权后完成首轮百炼/DeepSeek 复验，端到端 Top-5 为 72/120（60.00%）。该历史失败随后由 R4/R5 修复，不代表当前 P03-A11 状态。

## Historical R6 Live Golden Dataset Quality
...
- R6 首轮 P03-A11 Top-5 Recall 为 72/120（60.00%），当时 FAIL；精确 Chunk 命中 72 条，同文档命中 88 条。当前状态见后续 R4/R5 小节。
...
- 候选池规模最少 13、最多 59、平均 42.275；它不是 Reranker Top-5，因此在该 R4 检查点仍保持 FAIL。后续获批重排与 R5 结论见下一小节。
- 新增 `--retrieval-only --embedding-cache <完整本地缓存>` 模式；缓存缺失或 Hash 过期时失败关闭，确保下一轮只调用 Reranker，不重新调用 Embedding，也不调用 DeepSeek。

## P03-A11 R4 Live Reranker and R5 Protected Fusion

- 用户明确授权后，R4 将 120 条查询及每条 13~59 个候选片段发送至百炼 `qwen3-rerank`；Embedding 与 DeepSeek 均未调用。
- 120/120 个真实重排结果完成。一次读取超时由有限重试与逐条断点恢复处理；最终纯语义 Top-5 为 91/120（75.83%），同文档为 107/120（89.17%）。
- 失例诊断确认纯语义重排覆盖了 OCR 规范化后可精确命中的字面证据；R5 使用“百炼第 1 名 + 确定性词法前 4 名”的保护性融合，不读取金标、答案术语或 ChunkId。
- R5 在零新增外部调用下复用 120 条真实重排缓存，精确 Top-5 为 114/120（95.00%），同文档 118/120（98.33%）；GIN/HNSW 均命中，P03-A11 PASS。
...
## P03-A12 Prompt v2 Live Validation
...
- 用户明确同意仅把 120 条查询及每条 5 个 R5 Context 发送至 DeepSeek；本轮不调用百炼 Embedding 或 Reranker。120/120 条预测完整，两个瞬时空/非约束响应由逐条缓存断点续跑恢复。
...
- 49 份新增实际客户调研记录在本地完成解析、Schema、文件 Hash 和正文 Hash 去重；49/49 通过。
- 真实调研记录被标记为主要客户事实证据；调研业务表单标记为仅供参考，不可满足调研类独立样本配额。
- 50 条来源锁 PASS：标准能力 33、合同 7、技术协议 8、调研 2；调研 2/2 均来自新的实际记录。
- 历史暴露 Chunk、相邻来源定位、重复正文和 13 个表单参考块均被排除；141/141 单元测试 PASS，外部 AI 调用 0。
- 原文件名、客户名称、原始 Hash、正文、ChunkId、定位器和本地锁文件均未提交。

## Independent Holdout AI Review R1

- 获得当轮明确授权后，50 条锁定候选经统一 AIService 发送至 DeepSeek；不调用 Embedding 或 Reranker。
- 50/50 建议完成，问题 50/50 唯一，五类分类全覆盖；分类分布为 23/3/10/13/1。
- DeepSeek V4 默认思考模式导致 JSON 最终正文为空；显式关闭思考模式后，以受约束纯 JSON 恢复完成。累计请求尝试 227，失败响应未写入业务建议。
- 9 条摘录和 9 条关键词因同义改写未通过逐字落地校验，均由本地原文确定性替换并留有修复标记。
...
2. P03-A11 已以精确 Chunk 114/120（95.00%）通过；P03-A12/P03-A13 仍 FAIL，不得用检索通过、同文档命中或事后改标签替代分类/引用 Gate。
