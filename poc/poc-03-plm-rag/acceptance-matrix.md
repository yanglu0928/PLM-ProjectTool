...
|P03-A12|分类准确率 ≥90%|FAIL|NOT_RUN|NOT_RUN|Prompt v2 历史结果 51/120（42.50%）。50 条独立候选已生成 DeepSeek 建议，问题唯一且五类覆盖；当前均为待人工确认，不能用 AI 预填结果提前计分|
|P03-A13|来源引用准确率 ≥98%|FAIL|NOT_RUN|NOT_RUN|Prompt v2 历史结果 62/120（51.67%），越界引用 0。50 条独立候选均绑定未暴露 Chunk、来源定位和原文件入口，原文定位 50/50；待人工确认后才能形成引用真值|
