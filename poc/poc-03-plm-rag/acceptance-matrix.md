...
|P03-A12|分类准确率 ≥90%|FAIL|NOT_RUN|NOT_RUN|用户明确授权后，Prompt v2 复用完整本地 Embedding/R5 检索缓存完成 120/120 条真实 DeepSeek 预测，正确 51/120（42.50%），越界引用 0；Embedding 与当前轮 Reranker 外部调用均为 0。47 条 `INSUFFICIENT_INFORMATION` 中 30 条仍误判为 `STANDARD_SATISFIED`，6 条 `NON_STANDARD` 正确数仍为 0；继续同集调 Prompt 前必须重新确认问题与标签的可判定语义|
|P03-A13|来源引用准确率 ≥98%|FAIL|NOT_RUN|NOT_RUN|Prompt v2 真实来源引用正确 62/120（51.67%），越界引用 0；R5 精确检索已达 114/120，但模型只选中 62 个唯一期望 Chunk。验收仍按冻结的精确 Chunk 口径，不以同文档命中或扩展引用集合替代|
