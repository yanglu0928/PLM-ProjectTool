...
from poc03_rag.project_isolation import VECTOR_SQL, ProjectRetrievalRequest  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate pgvector HNSW Top-5 retrieval.")
    ...
    try:
        with connection.cursor() as cursor:
            ...
            records = []
            for scenario_id, query_vector in SCENARIOS:
                for number in range(1, 6):
                    records.append(
                        (
                            ...
                            "PROJECT",
                            "PROJECT-VECTOR",
                            "synthetic relevant record",
                            _vector_literal(query_vector),
                        )
                    )
            for number in range(1, 981):
                angle = (number * 0.61803398875) % (2 * math.pi)
                z = ((number * 37) % 200 - 100) / 100.0
                ...
                records.append(
                    (
                        ...
                        "PROJECT",
                        "PROJECT-VECTOR",
                        "synthetic distractor record",
                        _vector_literal((x, y, z)),
                    )
                )
            cursor.executemany(
                """
            ...
            cursor.execute(
                "CREATE INDEX poc03_vector_hnsw_idx "
        ...
        for scenario_id, query_vector in SCENARIOS:
            ...
            with connection.cursor() as cursor:
                cursor.execute(VECTOR_SQL, request.parameters)
                rows = cursor.fetchall()
                cursor.execute("EXPLAIN " + VECTOR_SQL, request.parameters)
            ...
            hnsw_index_used = hnsw_index_used and "poc03_vector_hnsw_idx" in plan
        ...
        status = (
            "PASS"
            if average_recall >= 0.95 and minimum_recall >= 0.95 and hnsw_index_used
            else "FAIL"
        )
        report = {
            "schema_version": "poc-03.vector-retrieval-result.v1",
