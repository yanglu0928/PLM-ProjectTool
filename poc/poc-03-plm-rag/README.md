...
`AWAITING_EXTERNAL_DATA_PROCESSING_APPROVAL`
...
剩余 6 条问题的唯一目标 Chunk 排名为 8、11、24、27、41、117；用户批准并确认方案 A 后，R6 只更新这 6 条的问题与引用，其余 114 条和全部 R5 分类保持不变。严格导入、覆盖审计均 PASS；本地确定性 Top-5 提升到 116/120（96.67%）。完整真实复验需要把 R6 查询与候选片段发送至百炼和 DeepSeek，当前等待本轮显式数据处理授权，尚未发送 R6 数据。
...
|Top-5 Recall|≥95%|R6 OCR 规范化确定性检索 96.67%（116/120），PASS；完整百炼重排待本轮外部数据处理授权|
|分类准确率|≥90%|R5 对既有预测重评分 57.50%（69/120），FAIL；R6 DeepSeek 复验尚未启动|
|来源引用准确率|≥98%|R6 本地 Top-5 上限 96.67%；完整百炼/DeepSeek 复验尚未启动，IN_PROGRESS|
...
1. R6 Golden Dataset 已确认并严格导入；P03-A11 达标。P03-A12/A13 的真实百炼与 DeepSeek 复验等待本轮显式数据处理授权。
