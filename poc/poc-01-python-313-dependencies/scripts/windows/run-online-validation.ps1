...
$runtimeRoot = Join-Path $repoRoot ".poc-runtime\poc-01\windows-online"
$evidenceRoot = Join-Path $pocRoot "evidence\windows-local"
...
& $python -m pip install --upgrade pip setuptools wheel 2>&1 |
    Tee-Object -FilePath (Join-Path $evidenceRoot "bootstrap.txt")
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $python -m pip install -r $requirements 2>&1 |
    Tee-Object -FilePath (Join-Path $evidenceRoot "install.txt")
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
