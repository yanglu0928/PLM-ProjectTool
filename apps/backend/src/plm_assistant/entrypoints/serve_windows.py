...
def main() -> int:
    if (sys.platform != "win32" or len(sys.argv) not in (2, 3)
            or (len(sys.argv) == 3 and sys.argv[2] != "--platform")):
        print("Usage: python -m plm_assistant.entrypoints.serve_windows <bootstrap.yaml> [--platform]", file=sys.stderr)
        return 2
    try:
        ...
        app_factory = (create_production_platform_app if len(sys.argv) == 3
