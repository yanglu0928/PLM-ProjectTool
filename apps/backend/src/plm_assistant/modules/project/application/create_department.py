...
class ProjectDepartmentCreateReceiptPort(Protocol):
...
class ProjectDepartmentCreateService:
    def __init__(self, *, unit_of_work: Callable[[], object], access: ProjectDepartmentCreateAccessPort,
                 ...
                 receipts: ProjectDepartmentCreateReceiptPort | None = None) -> None:
    ...
    def create_idempotent(self, command: CreateProjectDepartment, *,
                          idempotency_key: str) -> DepartmentView:
        if self._receipts is None:
            raise ProjectDepartmentCreateError("PROJECT_UNAVAILABLE")
        code, normalized_code, name = self._prepare(command)
        try:
            ...
            with self._uow() as tx:
                ...
                scope = IdempotencyScope.from_key(
                    ...
                    operation="V1_PROJECT_DEPARTMENT_CREATE", key=idempotency_key,
                )
                replay = self._receipts.reserve(
                    tx, scope=scope, request_fingerprint=fingerprint,
                )
                if replay is not None:
                    if (replay.ref_type != "V1_PROJECT_DEPARTMENT_CREATE"
                            or replay.status_code != 201):
                        raise ProjectDepartmentCreateError("PROJECT_UNAVAILABLE")
                    result = self._repository.get_create_result(
                        tx, project_id=command.project_id,
                        department_id=replay.ref_id,
                    )
                    if type(result) is not DepartmentView:
                        raise ProjectDepartmentCreateError("PROJECT_UNAVAILABLE")
                    return result
                self._require_create_authorization(tx, actor, command)
                result = self._create_locked(tx, command, actor, code, normalized_code, name)
                self._repository.save_create_result(
                    tx, project_id=command.project_id, view=result,
                )
                self._receipts.complete(
                    tx, scope=scope,
                    result=IdempotencyResult(
                        "V1_PROJECT_DEPARTMENT_CREATE", result.department_id, 201,
                    ),
                )
                tx.commit()
                return result
    ...
    @staticmethod
    def _prepare(command: CreateProjectDepartment) -> tuple[str, str, str]:
    ...
    def _authenticated_actor(self, tx: object, command: CreateProjectDepartment) -> uuid.UUID:
        now = self._clock()
    ...
    def _require_create_authorization(self, tx: object, actor: uuid.UUID,
                                      command: CreateProjectDepartment) -> None:
        try:
    ...
    def _create_locked(self, tx: object, command: CreateProjectDepartment,
                       actor: uuid.UUID, code: str, normalized_code: str,
                       name: str) -> DepartmentView:
