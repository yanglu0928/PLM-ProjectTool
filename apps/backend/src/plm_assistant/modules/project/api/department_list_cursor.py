"""Authenticated Project Department history keyset cursor."""
...
_FIELDS = {"v", "family", "scope", "session", "query", "department_id"}
...
class DepartmentListCursorCodec:
    def __init__(self, key: bytes) -> None:
        if type(key) is not bytes or len(key) != 32:
            raise ValueError("dedicated 32-byte department cursor signing key is required")
        self._key = key

    def encode(self, *, session_token: bytes, project_id: uuid.UUID,
               page_size: int, department_id: uuid.UUID) -> str:
        if (type(session_token) is not bytes or len(session_token) != 32
            ...
            raise ValueError("invalid department cursor position")
        payload = {
            "v": 1, "family": "project-department-history",
    ...
    def decode(self, token: str, *, session_token: bytes,
        ...
        try:
            ...
            if (type(payload) is not dict or set(payload) != _FIELDS
                    ...
                    or payload["family"] != "project-department-history"
            ...
            if (department_id.int == 0 or str(department_id) != payload["department_id"]
                    or self.encode(session_token=session_token, project_id=project_id,
                                   page_size=page_size, department_id=department_id) != token):
                raise ValueError()
