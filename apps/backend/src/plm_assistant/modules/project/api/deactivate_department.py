"""Opt-in Project Department deactivation with exact replay semantics."""
...
def _error(exc: ProjectDepartmentDeactivateError) -> ApplicationError:
    return ApplicationError({
        "AUTH_ACCESS_DENIED": "AUTH_SESSION_EXPIRED",
        "RESOURCE_NOT_FOUND": "RESOURCE_NOT_FOUND",
        "PROJECT_ARCHIVED": "PROJECT_ARCHIVED",
        "PROJECT_DEPARTMENT_IN_USE": "PROJECT_DEPARTMENT_IN_USE",
...
def create_project_department_deactivate_router(*, sessions: SessionService,
                                                departments: ProjectDepartmentDeactivateService,
                                                origins: LoginOriginPolicy) -> APIRouter:
    if sessions is None or departments is None or origins is None:
        raise ValueError("session, department deactivate service and origins are required")
    router = APIRouter()

    @router.post("/api/v1/projects/{project_id}/departments/{department_id}:deactivate")
    async def deactivate_department(project_id: uuid.UUID, department_id: uuid.UUID,
        ...
        command = DeactivateProjectDepartment(
            token, csrf, uuid.UUID(request.state.trace_id), project_id,
            department_id, expected_version,
        )
        try:
            view = await run_in_threadpool(
                departments.deactivate_idempotent, command, idempotency_key=key,
            )
            ...
            raise ApplicationError("SYSTEM_UNAVAILABLE") from None
        if type(view) is not DepartmentView or view.state != "INACTIVE":
