...
class ReviewSubjectTransitionPort(Protocol):
    ...
    def assert_terminal_consumed_in_transaction(self, tx: object, transition: ReviewSubjectTransition) -> None:
        """Independently re-read actual consumed result, formal pointer and released
        Owner identity lock. Only APPROVED may formalize the bound version;
        RETURNED/WITHDRAWN must retain prior approved version. Raise on mismatch.
