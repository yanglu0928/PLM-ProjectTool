...
class FakeUow:
    ...
    def commit(self) -> None:
        ...
        self.store.receipts = self.working.receipts
        self.committed = True
...
class FakeRepo:
    def add_configuration(self, uow, *, configuration_id, config_key, actor_id):
        if uow.working.root is not None:
            return False
        uow.working.root = ConfigurationSnapshot(
            configuration_id, config_key, 0, None
        )
        return True

    def lock(self, uow, configuration_id):
        root = uow.working.root
        return root if root is not None and root.configuration_id == configuration_id else None
...
class FakeReceipts:
    def reserve(self, uow, *, actor_id, operation, key_digest, request_fingerprint):
        identity = (actor_id, operation, key_digest)
        row = uow.working.receipts.get(identity)
        if row is None:
            uow.working.receipts[identity] = {
                "request_fingerprint": request_fingerprint, "result": None
            }
            return None
    ...
    def complete(self, uow, *, actor_id, operation, key_digest, result):
        uow.working.receipts[(actor_id, operation, key_digest)]["result"] = result
...
class ConfigurationCommandTests(unittest.TestCase):
    ...
    def _create(self, value="enabled", lock=0, schema=1, key=None):
        return self.service.create_version(CreateConfigurationVersion(
            CONFIG_ID, ACTOR_ID, lock, schema, value, key or str(uuid.uuid4())
        ))
    ...
    def test_activation_moves_only_root_pointer(self):
        ...
        result = self.service.activate_version(ActivateConfigurationVersion(
            CONFIG_ID, ACTOR_ID, 2, 1, str(uuid.uuid4())
        ))
    ...
    def test_activation_audit_failure_rolls_back_pointer(self):
        ...
        with self.assertRaisesRegex(ConfigurationCommandError, "SYSTEM_UNAVAILABLE"):
            self.service.activate_version(ActivateConfigurationVersion(
                CONFIG_ID, ACTOR_ID, 1, 1, str(uuid.uuid4())
            ))
    ...
    def test_missing_or_repeated_activation_is_rejected(self):
        with self.assertRaisesRegex(ConfigurationCommandError, "RESOURCE_NOT_FOUND"):
            self.service.activate_version(ActivateConfigurationVersion(
                CONFIG_ID, ACTOR_ID, 0, 1, str(uuid.uuid4())
            ))
        self._create()
        self.service.activate_version(ActivateConfigurationVersion(
            CONFIG_ID, ACTOR_ID, 1, 1, str(uuid.uuid4())
    ...
    def test_command_repr_excludes_value(self):
        command = CreateConfigurationVersion(
            CONFIG_ID, ACTOR_ID, 0, 1, "client-secret", str(uuid.uuid4())
        )
        self.assertNotIn("client-secret", repr(command))

    def test_create_identity_and_same_key_replay(self):
        self.store.root = None
        command = CreateConfiguration("app.mode", ACTOR_ID, str(uuid.uuid4()))
        first = self.service.create_configuration(command)
        second = self.service.create_configuration(command)
        self.assertEqual(first, second)
        self.assertEqual(first.lock_version, 0)
        self.assertIsNone(first.version_id)
        self.assertEqual(len(self.store.audit), 1)
        self.assertEqual(len(self.store.receipts), 1)

    def test_same_key_different_payload_conflicts_without_write(self):
        key = str(uuid.uuid4())
        first = self._create(key=key)
        with self.assertRaisesRegex(ConfigurationCommandError, "CONFLICT_IDEMPOTENCY"):
            self._create(value="disabled", key=key)
        self.assertEqual(len(self.store.versions), 1)
        self.assertEqual(self._create(key=key), first)

    def test_invalid_idempotency_key_fails_before_write(self):
        for key in ("short", "x" * 129, "中" * 16):
            with self.assertRaisesRegex(ConfigurationCommandError, "VALIDATION_FAILED"):
                self._create(key=key)
        self.assertEqual(self.store.versions, [])

    def test_create_duplicate_identity_with_other_key_conflicts(self):
        with self.assertRaisesRegex(ConfigurationCommandError, "CONFLICT_DUPLICATE"):
            self.service.create_configuration(CreateConfiguration(
                "app.mode", ACTOR_ID, str(uuid.uuid4())
            ))
