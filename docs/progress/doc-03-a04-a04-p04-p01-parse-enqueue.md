...
依据冻结 DM-03、ADR-007、`CR-DOC-006` 和 `DEC-20260926-116`，jobs 模块新增受限 `ParseJobQueue` Application Port。调用方提供已有短事务，入队接口只保存上传/文档版本 ID、Scope/Project、原 actor 与 trace 引用；在该事务内同时登记 `DOCUMENT_PARSE` Job 和 `DOCUMENT_VERSION_COMMITTED` Outbox，不自行提交、不访问 Document 内部表、不携带文件正文、Token、Secret 或 Prompt。相同 UploadId/版本再次调用返回原 Job/Event；同 UploadId 变更版本、actor 或 trace 拒绝。
