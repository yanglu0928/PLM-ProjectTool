...
|2026-09-21|用户明确授权将 50 条独立留出集的查询及最小必要候选正文发送至百炼 Embedding/Reranker 和 DeepSeek。首次装载发现 17 个重复 ChunkId、22 行冗余记录，核验均为逐字段完全一致；新增安全去重并保持冲突重复失败关闭。续跑完成 2,106 个唯一向量、50/50 条真实重排和 50/50 条预测。|
|2026-09-21|独立留出集质量结果：Top-5 49/50（98.00%）达到 95% 门槛；分类 24/50（48.00%）低于 90%；引用 37/50（74.00%）低于 98%。缺失预测、越界引用均为 0，GIN/HNSW 命中。P03-A11 PASS，P03-A12/P03-A13 和 POC-03 总体保持 FAIL；本轮 50 条冻结为已见测试集，后续修复需要全新独立留出集。|
|2026-09-21|R10 本地失败分层诊断完成：17 条需求来源样本检索命中 16 条而分类只命中 1 条，确认能力适配输入缺少跨来源标准能力对照；模型 46/50 次引用第 1 名，正确证据位于第 2～5 名时只命中 2/12；13 条严格引用失例中 8 条引用覆盖全部答案术语。冻结当前结果、不事后补标，下一轮采用独立开发集、双来源证据、Prompt v3 与 Evidence Selector。|
|2026-09-21|用户同意 R10 修复方向并要求不重复验证；R11 完成纯离线 Prompt v3、需求/标准能力双来源证据装配和 Evidence Selector。10 项合成测试验证问题类型路由、ProjectId 隔离、Golden 字段防泄漏和非首位证据选择；本轮外部调用 0、50 条留出集未重跑，POC-03 历史 FAIL 与质量 Gate 不变。|
