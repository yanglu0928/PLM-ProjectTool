...
版本：V2.1  
变更日期：2026-09-17  
变更内容：新增 Windows 11 x86-64 作为正式目标环境，与 Windows Server 2025、Debian 13 并列支持；相应调整 Phase 0、安装验收、发行和风险表述。  
历史版本：V2.0 保留，不覆盖。
...
- Windows 11 与 Windows Server 2025 为第一版 Windows 正式目标环境；
...
- 三个目标OS：Windows 11、Windows Server 2025、Debian 13；
...
- Windows 11 / Windows Server 2025离线包；
...
|---|---|---|---|---|---|---|
|P0.01|创建Python3.13 Windows 11测试环境|无|Windows 11|venv|Python正常|中|
|P0.01S|创建Python3.13 Windows Server测试环境|无|Server2025|venv|Python正常|中|
|P0.02|创建Python3.13 Debian环境|无|Debian13|venv|Python正常|中|
|P0.03|构建三平台离线wheel仓库|P0.01/P0.01S/P0.02|requirements|wheelhouse|三平台断网安装|高|
|P0.04|PaddleOCR兼容测试|P0.03|扫描PDF|OCR结果|中文准确|高|
|P0.05|PostgreSQL18 Windows安装矩阵|无|Windows 11/Server installer|DB|两种 Windows 均 offline|高|
|P0.06|pgvector Windows兼容矩阵|P0.05|extension|vector|两种 Windows 均 SQL通过|高|
...
Windows 11、Windows Server 2025、Debian 13各执行：
...
|R14|三平台离线包依赖复杂|native dependency|安装失败|高|高|wheel/deb bundle|PoC|
...
Python3.13全部依赖能否在 Windows 11、Windows Server 2025、Debian 13 三个平台离线安装。
...
```text
Windows 11 / Windows Server 2025离线发行包
Debian 13离线发行包
数据库初始化包
数据库升级脚本
Plugin包
